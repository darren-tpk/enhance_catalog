...
party_clean.plot()
party_clean.plot(plot_grouped=True)
...
num_day = 26#np.ceil((max(all_detection_time) - min(all_detection_time))/86400)
bin_counts = []
...
fig,ax = plt.subplots()
ax.grid(True)
ax.hist(range(int(num_day)),range(int(num_day+1)),weights=bin_counts,color='royalblue',edgecolor='black')
ax.set_xlim([0,26])
ax.set_ylim([0,170])
...
hypoi_file = '/Users/darrentpk/Desktop/avo_data/redoubt_20090215_20090316_hypoi.txt'
hypoddpha_file = '/Users/darrentpk/Desktop/avo_data/redoubt_20090215_20090316_hypoddpha.txt'
...
# Bin by day
num_day = 26 #np.ceil((max(all_event_time) - start_day)/86400)
bin_counts = []
for i in range(int(num_day)):
    bin_start = start_day + i*86400
    bin_end = start_day + (i+1)*86400
...
fig,ax = plt.subplots()
ax.grid(True)
ax.hist(range(int(num_day)),range(int(num_day+1)),weights=bin_counts,color='indianred',edgecolor='black')
ax.set_xlim([0,26])
ax.set_ylim([0,170])
ax.set_xlabel('Days since ' + str(start_day)[:10]);
ax.set_ylabel('Daily frequency');
...
cat_family = Family(template=Template(name='original_catalog'))
for detect_time in aligned_event_time:
    ...
    cat_family = cat_family + detection
