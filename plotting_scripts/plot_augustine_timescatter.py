...
main_dir = '//'
output_dir = main_dir + 'output/augustine2/'
...
relocated_catalog_filepath = output_dir + 'relocate_catalog/augustine_hypodd_relocated_node_sampled.xml'
...
start_time = UTCDateTime(2006, 3, 13)
end_time = UTCDateTime(2006, 3, 16)
tick_precision = '6hour' # '1hour', '6hour','day','month'
event_count_style = 'linear' # 'log', 'linear'
plot_explosions = True
...
for event in relocated_catalog:
    az12, az21 ,dist12 = geodesic.inv(aug_lon, aug_lat, event.origins[0].longitude, event.origins[0].latitude)
    if az12 < 0:
        faz.append(az12 + 360)
    else:
        faz.append(az12)
...
title = 'Augustine Seismicity Trends, %s to %s' % (tick_times[0].strftime('%d %b %Y'),tick_times[-1].strftime('%d %b %Y'))
