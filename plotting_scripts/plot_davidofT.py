...
output_dir = main_dir + 'output/davidof/'
...
start_time = UTCDateTime(2021, 12, 1)
end_time = UTCDateTime(2021, 12, 31)
tick_precision = 'day' # '1hour', '6hour','day','month'
...
for event in relocated_catalog:
    az12, az21 ,dist12 = geodesic.inv(dvf_lon, dvf_lat, event.origins[0].longitude, event.origins[0].latitude)
    if az12 < 0:
        faz.append(az12 + 360)
    else:
        faz.append(az12)
...
if tick_precision == 'month' or tick_precision == 'day':
...
axs[2].set_yticks([-1.0,-0.5,0,0.5])
...
axs[4].set_ylim([-3.5,11])
axs[4].set_yticks([-3,0,3,6,9])
axs[4].invert_yaxis()
...
start_time = UTCDateTime(2021, 12, 1)
end_time = UTCDateTime(2021, 12, 31)
tick_precision = '7day' # '1hour', '6hour','day','month'
event_count_style = 'linear' # 'log', 'linear'
...
mag = [event.magnitudes[0].mag for event in catalog]
depth = [event.origins[0].depth / 1000 for event in catalog]
fi = [float(event.comments[-1].text.split('=')[1]) for event in catalog]
station_coords = [(event.origins[0].latitude,event.origins[0].longitude) for event in catalog]
dist = []
faz = []
baz = []
geodesic = Geod(ellps='WGS84')
...
all_times = [event.origins[0].time for event in catalog]
days = [(time-start_time)/86400 for time in all_times]
num_days = int((tick_times[-1] - tick_times[0])/86400)
...
fig, axs = plt.subplots(6, 1, sharex=True,figsize=(9,10))
fig.subplots_adjust(hspace = 0.25)
if event_count_style == 'log':
    axs[0].semilogy(days_evcount,evcounts,'k-')
    axs[0].set_ylim([1,10000])
...
if plot_explosions:
    for ax_number in range(1,7):
        for day_exp in days_exp:
            axs[ax_number].axvline(x=day_exp, color='dimgray', alpha=0.5, zorder=0)
axs[1].scatter(days,fi,s=18,c='red',marker='.',alpha=0.4)
axs[1].set_ylim([-1,0.5])
axs[1].set_yticks([-1.0,-0.5,0,0.5])
axs[1].set_ylabel('FI',fontsize=13)
axs[1].tick_params(axis='y',labelsize=13)
axs[2].scatter(days,mag,s=18,c='black',marker='.',alpha=0.4)
axs[2].set_ylim([-1,5])
axs[2].set_yticks([-1,1,3,5])
axs[2].set_ylabel('Magnitude',fontsize=13)
axs[2].tick_params(axis='y',labelsize=13)
axs[3].scatter(days,depth,s=18,c='darkblue',marker='.',alpha=0.4)
axs[3].set_ylim([-3.5,11])
axs[3].set_yticks([-3,0,3,6,9])
axs[3].invert_yaxis()
axs[3].set_ylabel('Depth (km)',fontsize=13)
axs[3].tick_params(axis='y',labelsize=13)
axs[4].scatter(days,dist,s=18,c='darkolivegreen',marker='.',alpha=0.4)
axs[4].set_ylim([0,21])
axs[4].set_yticks([0,7,14,21])
axs[4].set_ylabel('Dist (km)',fontsize=13)
axs[4].tick_params(axis='y',labelsize=13)
axs[5].scatter(days,faz,s=18,c='rebeccapurple',marker='.',alpha=0.4)
axs[5].set_ylim([0,360])
axs[5].set_yticks([0,90,180,270,360])
axs[5].set_ylabel('Azimuth (°)',fontsize=13)
axs[5].tick_params(axis='y',labelsize=13)
axs[5].set_xlim([0,num_days])
axs[5].set_xticks((np.array(tick_times) - start_time) / 86400)
if tick_precision == '1hour':
    axs[5].set_xticklabels([tick_time.strftime('%m/%d %H:00') for tick_time in tick_times], fontsize=13, rotation=30, ha='right')
elif tick_precision == '6hour':
    axs[5].set_xticklabels([tick_time.strftime('%m/%d %H:00') for tick_time in tick_times], fontsize=13, rotation=30, ha='right')
elif tick_precision == 'day':
    axs[5].set_xticklabels([tick_time.strftime('%m/%d') for tick_time in tick_times], fontsize=13, rotation=30, ha='right')
elif tick_precision == '7day':
    axs[5].set_xticklabels([tick_time.strftime('%m/%d') for tick_time in tick_times], fontsize=13, rotation=30, ha='right')
elif tick_precision == 'month':
    axs[5].set_xticklabels([tick_time.strftime('%b \'%y') for tick_time in tick_times], fontsize=13, rotation=30, ha='right')
else:
    raise ValueError('wrong tick precision setting')
# save_path = output_dir + 'seismic_trends_' + start_time.strftime('%Y%m%d') + '_' + end_time.strftime('%Y%m%d')+ '.png'
# plt.savefig(save_path)
