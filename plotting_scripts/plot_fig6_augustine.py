...
catalog_filepath = main_dir + 'output/augustine5/scan_data/party_FI.xml'
catalog = reader(catalog_filepath)
relocated_catalog_filepath = main_dir + 'output/augustine5/scan_data/relocatable_catalog_FImag.xml'
relocated_catalog = reader(relocated_catalog_filepath)
for event in relocated_catalog:
    if event.magnitudes == []:
        relocated_catalog.events.remove(event)
original_filepath = main_dir + 'output/augustine5/scan_data/PEC_FI.xml'
...
color_code_table = read_csv(main_dir + 'data/supporting/aug_color_codes.csv')
color_codes = []
color_code_times = []
...
volc_tick_lims = [UTCDateTime(2005, 4, 1), UTCDateTime(2005, 5, 1),
                  UTCDateTime(2005, 6, 1), UTCDateTime(2005, 7, 1),
                  UTCDateTime(2005, 8, 1), UTCDateTime(2005, 9, 1),
                  UTCDateTime(2005, 10, 1), UTCDateTime(2005, 11, 1),
                  UTCDateTime(2005, 12, 1), UTCDateTime(2006, 1, 1),
                  UTCDateTime(2006, 2, 1), UTCDateTime(2006, 3, 1),
                  UTCDateTime(2006, 4, 1), UTCDateTime(2006, 5, 1)]
volc_bin_lims = [UTCDateTime(2005, 4, 1)]
while volc_bin_lims[-1] < UTCDateTime(2006, 5, 1):
    volc_bin_lims.append(volc_bin_lims[-1] + (86400*7))
volc_base_time = UTCDateTime(2005,4,1)
...
ax0b = ax0.twinx()
ax0b.plot(cum_volc_PARTY_days,cum_volc_PARTY_events,'-',color='black',linewidth=8,label='Augustine EQcorrscan')
ax0b.plot(cum_volc_PEC_days,cum_volc_PEC_events,'-',color='darkred',linewidth=8,label='Augustine Original Catalog',zorder=9)
ax0b.plot(cum_volc_PARTY_days[-1],cum_volc_PARTY_events[-1],'o',color='black')
ax0b.plot(cum_volc_PEC_days[-1],cum_volc_PEC_events[-1],'o',color='darkred')
# ax0b.legend(fontsize=32,loc='lower left')
ax0b.set_ylim([0,5000])
...
base_time = UTCDateTime(2005,4,1)
end_time = UTCDateTime(2006,5,1)
...
zoom1_base_time = UTCDateTime(2006,1,7)
zoom1_end_time = UTCDateTime(2006,1,29)
zoom2_base_time = UTCDateTime(2006,3,8)
zoom2_end_time = UTCDateTime(2006,3,16)
tick_times = [UTCDateTime(2005, 4, 1), UTCDateTime(2005, 5, 1),
              UTCDateTime(2005, 6, 1), UTCDateTime(2005, 7, 1),
              UTCDateTime(2005, 8, 1), UTCDateTime(2005, 9, 1),
              UTCDateTime(2005, 10, 1), UTCDateTime(2005, 11, 1),
              UTCDateTime(2005, 12, 1), UTCDateTime(2006, 1, 1),
...
ax1.scatter(relocated_days, relocated_FI_values, s=relocated_sizes, c='indianred', alpha=0.5)
ax1.scatter(original_days, original_FI_values, s=original_sizes, c='darkred', alpha=0.7)
ax1.scatter(8, -1.05, s=750, marker='s', c='#505050', alpha=0.5)
ax1.scatter(8, -0.85, s=750, marker='s', c='indianred', alpha=0.5)
ax1.scatter(8, -0.65, s=750, marker='s', c='darkred')
ax1.scatter(8, -1.05, s=750, marker='s', c='None', edgecolor='k')
ax1.scatter(8, -0.85, s=750, marker='s', c='None', edgecolor='k')
ax1.scatter(8, -0.65, s=750, marker='s', c='None', edgecolor='k')
ax1.text(10.7, -1.05, ': Enhanced Catalog w/o Magnitudes', fontsize=35, va='center')
ax1.text(10.7, -0.85, ': Enhanced Catalog with Magnitudes', fontsize=35, va='center')
ax1.text(10.7, -0.65, ': Original AVO Catalog', fontsize=35, va='center')
ax1.scatter(8, -0.40, s=50, marker='o', c='None', edgecolor='k')
ax1.scatter(17, -0.40, s=315, marker='o', c='None', edgecolor='k')
ax1.scatter(26, -0.40, s=584, marker='o', c='None', edgecolor='k')
ax1.scatter(35, -0.40, s=852, marker='o', c='None', edgecolor='k')
ax1.text(8, -0.20, 'M0', fontsize=35, va='center', ha='center')
...
for k in range(len(color_code_days) - 1):
    ax1.axhspan(ymin=1.06, ymax=1.24, xmin=color_code_days[k] / num_days, xmax=color_code_days[k + 1] / num_days,
               color=color_codes[k], zorder=10)
    ax1.axhspan(ymin=-1.30, ymax=-1.25, xmin=(zoom1_base_time - base_time) / (86400 * num_days),
                xmax=(zoom1_end_time - base_time) / (86400 * num_days), color='blue', alpha=0.5)
    ax1.axhspan(ymin=-1.30, ymax=-1.25, xmin=(zoom2_base_time - base_time) / (86400 * num_days),
...
color_code_days = [(color_code_time - base_time) / 86400 for color_code_time in color_code_times]
for k in range(len(color_code_days) - 1):
    ax2.axhspan(ymin=0.63, ymax=0.75, xmin=color_code_days[k] / num_days, xmax=color_code_days[k + 1] / num_days,
               color=color_codes[k], zorder=10)
# Zoom 2 plot
...
# ax3.set_ylabel('Frequency Index',fontsize=38)
...
color_code_days = [(color_code_time - base_time) / 86400 for color_code_time in color_code_times]
for k in range(len(color_code_days) - 1):
    ax3.axhspan(ymin=0.63, ymax=0.75, xmin=color_code_days[k] / num_days, xmax=color_code_days[k + 1] / num_days,
               color=color_codes[k], zorder=10)
plt.tight_layout()
plt.show()
