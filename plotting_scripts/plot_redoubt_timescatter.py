...
party_catalog_filepath = output_dir + 'scan_data/party_catalog_full_reFI.xml'
relocated_catalog_filepath = output_dir + 'relocate_catalog/sub_catalog_layer.xml'
relocated_catalog_filepath2 = output_dir + 'relocate_catalog/may_swarm_layer.xml'
PEC_events = reader(PEC_events_filepath)
raw_party_catalog = reader(party_catalog_filepath)
sub_catalog = reader(relocated_catalog_filepath)
may_swarms = reader(relocated_catalog_filepath2)
raw_relocated_catalog = sub_catalog.copy() + may_swarms.copy()
rdt_dome_lat = 60.490
rdt_dome_lon = -152.7626
volcano_coord = (rdt_dome_lat,rdt_dome_lon)
...
start_time = UTCDateTime(2009, 3, 20, 18)
end_time = UTCDateTime(2009, 3, 23, 12)
tick_precision = '6hour' # '1hour', '6hour','day','month'
...
for event in relocated_catalog:
    az12, az21 ,dist12 = geodesic.inv(rdt_dome_lon, rdt_dome_lat, event.origins[0].longitude, event.origins[0].latitude)
    if az12 < 0:
        faz.append(az12 + 360)
    else:
        faz.append(az12)
...
title = 'Redoubt Seismicity Trends, %s to %s' % (tick_times[0].strftime('%d %b %Y'),tick_times[-1].strftime('%d %b %Y'))
...
axs[2].set_ylim([-1.86,0.51])
axs[2].set_yticks([-1.5,-1.0,-0.5,0,0.5])
...
axs[5].set_ylim([0,10.0])
axs[5].set_yticks([0,3,6,9])
