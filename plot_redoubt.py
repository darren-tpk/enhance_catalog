...
relocated_catalog_filepath = main_dir + 'output/redoubt2/relocate_catalog/relocated_catalog.xml'
...
REGION_WE = [0, 40, -5, MAX_DEPTH]
REGION_NS = [0, 40, -5, MAX_DEPTH]
...
PEC_hypoi = PEC_events_dir + 'redoubt_20080501_20090901_hypoi.txt'
PEC_hypoddpha = PEC_events_dir + 'redoubt_20080501_20090901_hypoddpha.txt'
...
if size_by_magnitude:
    ...
    for magnitude in magnitudes:
        if magnitude < min_mag_marker:
            ...
            size = 0.06 + normalized_magnitude*0.21
        sizes.append(size)
...
if plot_temporal:
    ...
    with fig.subplot(nrows=1, ncols=1, figsize=("8.5c","8.5c")):
        ...
        with pygmt.config(FONT_ANNOT_PRIMARY="7p,Helvetica,black"):
            fig.colorbar(position="JBL+o-8.5c/0.5c+w8.5c/0.4c+h",frame="xa1oyf")
        ...
        if size_by_magnitude:
            ...
            fig.plot(x=-153.05, y=60.32, color='gray', style="c0.122c", pen="black")
            fig.plot(x=-153.00, y=60.32, color='gray', style="c0.184c", pen="black")
            fig.plot(x=-152.95, y=60.32, color='gray', style="c0.245c", pen="black")
        ...
        fig.plot(x=WE_x, y=WE_y, pen="0.9p,black,-", transparency=40)
        fig.text(x=W[1], y=W[0], text="A", justify="RT", font="11p,black")
        fig.text(x=E[1], y=E[0], text="B", justify="LT", font="11p,black")
        fig.plot(x=NS_x, y=NS_y, pen="0.9p,black,-", transparency=40)
        fig.text(x=N[1], y=N[0], text="C", justify="RM", font="11p,black")
        fig.text(x=S[1], y=S[0], text="D", justify="RM", font="11p,black")
    ...
    with fig.subplot(nrows=1, ncols=1, figsize=("4c", "10c")):
        ...
        # Create basemap with correct dimensions
        fig.basemap(region=REGION_WE, projection="X4c/-10c", frame=['xa10f2+l"Distance (km)"', "ya10f2", "wsNe"],
                    panel=[0, 0])

        # # Plot landfill
        # for i in range(1, len(WE)):
        #     delta = (WE_elev_distances[i] - WE_elev_distances[i - 1]) / (REGION_WE[1]-REGION_WE[0]) * 4  # inter-measurement width
        ...
        #     data = [[WE_elev_distances[i], midpoint, delta, height_cm]]
        #     fig.plot(data=data, style="r", color="gray90", pen="1p,gray90")

        # Plot earthquakes
        pygmt.makecpt(cmap="viridis", series="2008-05-01T/2009-09-01T/1d")
        if size_by_magnitude:
            fig.plot(x=WE_down_distances, y=depths, size=sizes, style="c", color=times, cmap=True, pen="black", transparency=20)
        else:
            fig.plot(x=WE_down_distances, y=depths, style="c0.07c", color=times, cmap=True, pen="black", transparency=20)
        ...
        fig.plot(x=WE_elev_distances, y=-0.001*WE["Elev(m)"], pen="1.5p,black")
        fig.text(x=0.5, y=-4.5, text="A", justify="LT", font="13p,black")
        fig.text(x=39.5, y=-4.5, text="B", justify="RT", font="13p,black")
    ...
    with fig.subplot(nrows=1, ncols=1, figsize=("4c", "10c")):
        ...
        # Create basemap with correct dimensions
        fig.basemap(region=REGION_NS, projection="X4c/-10c", frame=['xa10f2+l"Distance (km)"', 'ya10f2+l"Depth (km)"', "wsNE"],
                    panel=[0, 0])

        # # Plot landfill
        # for i in range(1, len(NS)):
        #     delta = (NS_elev_distances[i] - NS_elev_distances[i - 1]) / (REGION_NS[1]-REGION_NS[0]) * 4  # inter-measurement width
        #     height_km = (MAX_DEPTH + 0.001 * NS["Elev(m)"][i])  # depth - negative elevation
        #     height_cm = height_km / (MAX_DEPTH+5) * 9.9  # ratio * figure height
        #     midpoint = MAX_DEPTH - 0.5 * height_km
        #     data = [[NS_elev_distances[i], midpoint, delta, height_cm]]
        #     fig.plot(data=data, style="r", color="gray90", pen="1p,gray90")

        # Plot earthquakes
        pygmt.makecpt(cmap="viridis", series="2008-05-01T/2009-09-01T/1d")
        if size_by_magnitude:
            fig.plot(x=NS_down_distances, y=depths, size=sizes, style="c", color=times, cmap=True, pen="black", transparency=20)
        else:
            fig.plot(x=NS_down_distances, y=depths, style="c0.07c", color=times, cmap=True, pen="black", transparency=20)
        ...
        # Plot elevation profile
        fig.plot(x=NS_elev_distances, y=-0.001 * NS["Elev(m)"], pen="1.5p,black")
        fig.text(x=0.5, y=-4.5, text="C", justify="LT", font="13p,black")
        fig.text(x=39.5, y=-4.5, text="D", justify="RT", font="13p,black")
