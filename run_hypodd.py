...
output_dir = main_dir + 'output/augustine3/relocate_catalog/'
...
# Choose operations
xml_filepath = hypoddver_dir + 'IN/relocatable_catalog_FImag.xml'
# xml_filepath = main_dir + 'output/augustine/scan_data/relocatable_catalog.xml'
...
## hyppoDD.inc
MAXEVE = 5000  # Maximum number of events; recommended: large: 6500, small: 1000
MAXDATA = 3000000  # Maximum number of observations; recommended: large: 3000000, small: 20000
...
MAXSEP = 10  # Maximum hypocentral separation between event pairs; out-of-box: 10
...
MAXOBS = 20  # Maximum number of links per pair saved; out-of-box: 100
## hypoDD.inp
#### IN
...
stlist_file = './hypoDDver/IN/stations.dat'
vzmodel_file = './data/vz/augustine_nodemodel_sampled.txt'  # NOT parsed into hypodd directly
...
WTCCP = [0.001, 0.001, 1, 1] # [0.01, 0.01, 0.10, 0.50, 1.00]  # Weight for cc P wave (-9: omit)
WTCCS = [0.001, 0.001, 0.8, 0.8]  # [0.01, 0.01, 0.10, 0.50, 1.00]  #Weight for cc S wave (-9: omit)
WRCC = [10, 10, 10, 6]  # [10, 10, 10, 6, 6]  # Cutoff threshold for outliers located on cc tails (-9: no outlier removed)
WDCC = [4, 4, 4, 0.5]  # [4, 4, 4, 2, 2] #Maximum event separation for cc data (-9: not activated)
WTCTP = [1, 1, 0.001, 0.001] #[1.00, 1.00, 1.00, 0.10, 0.01]  # Weight for ct P wave (-9: omit)
WTCTS = [0.8, 0.8, 0.001, 0.001] #[1.00, 1.00, 1.00, 0.10, 0.01]  # Weight for ct S wave (-9: omit)
WRCT = [12, 6, 6, 6]  # [12, 6, 6, 6, 6]  # Cutoff threshold for outliers located on ct tails (-9: no outlier removed)
WDCT = [8, 3, 3, 3]  # [10, 5, 2.5, 2.5, 2.5]  #Maximum event separation for ct data (-9: not activated)
DAMP = [100, 100, 100, 100] # [80, 60, 80, 60]  # [100, 100, 100, 100, 100]  #Damping (only for LSQR, ISOLV=2)
###### 1D velocity model
...
def loc2cat(loc_filepath, event_id_mapper=None, input_catalog=None, type='loc', depth_correction=0):
    ...
    with open(loc_filepath, "r") as loc:
        ...
        for line in lines[:-1]:
            ...
            lon = float(lon)
            lat = float(lat)
            dep = (float(dep) - depth_correction) * 1000
            mag = float(mag)
    ...
    with open(dat_filepath, "r") as dat:
        ...
        for line in lines[:-1]:
            ...
            ev = Event(origins=[Origin(time=time, longitude=float(lon), latitude=float(lat),
                                       depth=(float(dep) - depth_correction) * 1000)],
...
for event in catalog:
    ...
    if correct_depths:
        dep = event.origins[0].depth / 1e3 + depth_correction
    else:
        dep = event.origins[0].depth / 1e3
    if event.magnitudes == []:
        mag = -9
    else:
