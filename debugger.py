...
hypoi_path = '/home/ptan/enhance_catalog/data/avo/augustine_20050401_20060501_hypoi.txt'
hypoddpha_path = '/home/ptan/enhance_catalog/data/avo/augustine_20050401_20060501_hypoddpha.txt'
catalog = read_hypoddpha(hypoi_path,hypoddpha_path,'True')

for event in catalog:
    if event.origins[0].time > UTCDateTime(2005,12,16) and event.origins[0].time < UTCDateTime(2005,12,18):
        print(event.origins[0].time)

all_times = [
'2006-01-26T04:38:22.250000Z',
'2006-01-28T08:21:57.710000Z',]

# check SHZ vs EHZ data quality
data_dir = '/home/data/augustine/'
files = []
# station_list = ['AUH','AUL','AUP','AUE','AUI','AUW','AUS']
station_list = ['AUE','AUH','AU13','AUI','AUW']
campaign_list = ['AU11','AU13','AU14','AU15']

for time in all_times:
    year = time[0:4]
    julday = '%03d' % (UTCDateTime(time.split('T')[0]).julday)

    # find and plot EHZ
    files = []
    for station in station_list:
        if station in campaign_list:
            file_list = glob(data_dir + station + '.HHZ.' + year + ':' + julday + ':*')
        else:
            file_list = glob(data_dir + station + '.EHZ.' + year + ':' + julday + ':*')
        for file in file_list:
            files.append(file)
    stream = Stream()
    for file in files:
        stream_contribution = read(file)
        stream += stream_contribution
    stream = stream.trim(starttime=UTCDateTime(time) - 2, endtime=UTCDateTime(time) + 8)
    stream.filter("bandpass",freqmin=1,freqmax=8)
    try:
        stream.merge()
    except:
        ...
        stream.merge()
    stream.normalize()
    stream.plot(color='red')

from glob import glob
from obspy import read
from os import remove
file_list = glob(data_dir + '*.SH*')
for i, file in enumerate(file_list):
    print('Now at: ' + str(i) + '/' + str(len(file_list)))
    st = read(file)
    for tr in st:
        current_chan = tr.stats.channel
        new_chan = 'E' + current_chan[1:]
        tr.stats.channel = new_chan
    current_file = file
    file_pieces = current_file.split('.')
    file_pieces[1] = new_chan
    new_file = '.'.join(file_pieces)
    st.write(new_file,format='MSEED')
    remove(file)
...
with open('/home/ptan/enhance_catalog/output/redoubt2/relocate_catalog/out.growclust_boot') as f:
    lines = f.readlines()
...
for line in lines:
    if line.split(' ')[-1] == 'failed.\n':
        all_fails.append(int(line.split(' ')[1])-1)